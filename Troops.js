var strVersion = 'v8.6 (Discord Mod + BugFix)';
...
// -------------------------------------------------------------------------
// FUNÇÃO REESCRITA: AGORA LÊ CORRETAMENTE SEM APAGAR LINHAS
// -------------------------------------------------------------------------
function fnGetTroopCount() {
    var villageTroopInfo = [];

    // Descobre inteligentemente a ordem das colunas
    let unitColumns = [];
    ...
    jQuery('#units_table tbody tr').each(function() {
        ...
        // Na vista detalhada, o RedAlert só usa a linha "Próprias" para calcular Nukes
        ...
        var villageData = {
            ...
            coords: coords[0],
            troops: new Array(configUnits.length).fill(0)
        };
...
function fnSendToDiscord() {
    ...
    jQuery('#units_table thead th').each(function(index) {
        ...
        if (img) {
            for (let i = 0; i < game_data.units.length; i++) {
                if (img.includes('unit_' + game_data.units[i])) { unitColumns[index] = game_data.units[i]; discordData.total[game_data.units[i]] = 0; discordData.inside[game_data.units[i]] = 0; discordData.outside[game_data.units[i]] = 0; break; }
            }
        }
    });
...
function fnCalculateTroopCount() {
    ...
    docSource += '<div style="padding: 10px 0;"><button id="btnSendDiscord" class="btn btn-default" style="width: 100%; background-color: #5865F2; color: white; padding: 10px; border-radius: 4px; font-weight: bold; border: none; cursor: pointer; transition: 0.2s;">📤 Enviar para o Discord</button></div>';
    docSource += `<small><strong>Troops Counter ${strVersion}</strong> - Modified for Discord</small></body></html>`;

    if (DRAGGABLE) {
        docSource += `<a class="popup_box_close custom-close-button" onClick="clickDraggableEl();" href="#">&nbsp;</a><style>.troops-counter-content { position: fixed; background-color: #f4e4bc !important; border: 1px solid #603000; padding: 10px; top: 10vh; right: 15vw; z-index: 1000; width: 440px; height: auto; } .custom-close-button { right: 0; top: 0; } #btnSendDiscord:hover { background-color: #4752C4 !important; }</style>`;
        if (jQuery('.troops-counter-content').length > 0) {
            UI.ErrorMessage('Troops Counter is already loaded!');
        } else {
            var troopsCounterContent = document.createElement('div'); troopsCounterContent.classList.add('troops-counter-content'); troopsCounterContent.innerHTML = docSource; jQuery('body').append(troopsCounterContent); jQuery('.troops-counter-content').draggable({ cancel: '.not-draggable' }); jQuery('#btnSendDiscord').on('click', fnSendToDiscord);
        }
    } else {
        const popupContent = preparePopupContent(docSource, '440px'); Dialog.show('content', popupContent); jQuery('#btnSendDiscord').on('click', fnSendToDiscord); jQuery('head').append('<style>#btnSendDiscord:hover { background-color: #4752C4 !important; }</style>');
    }
}

function clickDraggableEl() { jQuery('.troops-counter-content').remove(); }
function initDebug() { console.debug(`[Troops Counter] Script Loaded`); }
function formatAsNumber(number) { return parseInt(number).toLocaleString('pt-PT'); }
function checkScreen(userScreen, userMode) { return (game_data.screen === userScreen || window.location.href.includes('screen=' + userScreen)) && (game_data.mode === userMode || window.location.href.includes('mode=' + userMode)); }
function preparePopupContent(popupBody, maxWidth) { return `<div class="ra-body" style="max-width: ${maxWidth}">${popupBody}</div>`; }

(async function () {
    if (!game_data.features.Premium.active) { UI.ErrorMessage('A Conta Premium precisa de estar ativa para este script funcionar!', 6000); } 
    else { fnExecuteScript(); }
})();
